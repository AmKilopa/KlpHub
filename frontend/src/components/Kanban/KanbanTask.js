...
export default function KanbanTask({
  task, colKey,
  askDelete, dragStartTask,
  isTaskSelected, toggleSelected,
  loadingIds,
  updateTaskTitleDesc,
}) {
  const isLoading = loadingIds && loadingIds.includes(task.id);
  ...
  // Наводим – подсветка; двойной клик — редактирование, при этом выключаем подсветку.
  ...
  const save = async () => {
    ...
    await updateTaskTitleDesc(task.id, title, localDesc);
    setEditMode(null);
    setHoverTitle(false);
    setHoverDesc(false);
  };
